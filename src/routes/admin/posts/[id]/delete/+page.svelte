  ...
  const handleEnhance = () => {
    return async ({ result }: { result: any }) => {
      if (result.type === 'failure') {
        error = result.data?.message || 'Failed to delete post. Please try again.';
      } else if (result.type === 'success') {
        // Show success message and redirect
        window.location.href = '/admin/posts?deleted=true';
      }
    };
  };
</script>
...
<svelte:head>
  <title>Delete Post | Admin | Ian Wilson</title>
</svelte:head>

<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-semibold text-gray-900">Delete Post</h1>
  </div>
