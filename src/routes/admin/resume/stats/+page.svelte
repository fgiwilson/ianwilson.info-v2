  ...
  const handleSubmit: SubmitFunction = () => {
    return async ({ result, formElement }) => {
      ...
      if (typedResult.success) {
        showSuccessMessage('Stat added successfully');
        formElement.reset();
      } else {
        // Handle error
...
<div class="container mx-auto px-4 py-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">Resume Stats</h1>
  ...
  <div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <h2 class="text-xl font-semibold mb-4">Resume Stats</h2>
          ...
          <tbody>
            {#each data.stats as stat}
              <tr class="border-t">
                ...
                <td class="py-2 px-4">{stat.order}</td>
                <td class="py-2 px-4 flex space-x-2">
                  <a href="/admin/resume/stats/{stat.id}/edit" class="text-blue-600 hover:underline">Edit</a>
                  <a href="/admin/resume/stats/{stat.id}/delete" class="text-red-600 hover:underline">Delete</a>
                </td>
              </tr>
            {/each}
          </tbody>
    ...
    {:else}
      <p class="text-gray-500">No stats found. Add some below.</p>
    {/if}
  </div>
  
  <div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-xl font-semibold mb-4">Add New Stat</h2>
