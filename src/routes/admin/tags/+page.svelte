...
<div class="container mx-auto px-4 py-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <table class="min-w-full bg-white">
            <thead>
              <tr class="bg-gray-100">
                ...
                <th class="py-2 px-4 text-left">Posts</th>
            ...
            <tbody>
              {#each data.tags as tag}
                <tr class="border-t">
                  ...
                  <td class="py-2 px-4">{tag.slug}</td>
                  <td class="py-2 px-4">{tag.posts?.length || 0}</td>
                  <td class="py-2 px-4 flex space-x-2">
                    <a href="/admin/tags/{tag.id}/edit" class="text-blue-600 hover:underline">Edit</a>
                    <form 
                      method="POST" 
                      action="?/delete" 
                      use:enhance={() => {
      ...
      {:else}
        <p class="text-gray-500">No tags found. Add some using the form.</p>
      {/if}
    </div>
