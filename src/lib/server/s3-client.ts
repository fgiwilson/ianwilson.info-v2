...
const isLinode = env.S3_ENDPOINT?.includes('linodeobjects.com');
const usePathStyle = env.S3_PATH_STYLE === 'true' || isLinode;

// Initialize S3 client with environment variables
const s3Client = new S3Client({
	region: env.S3_REGION || 'us-lax-1',
...
export function isS3Configured(): boolean {
	return !!(env.S3_ACCESS_KEY && env.S3_SECRET_KEY && env.S3_BUCKET && env.S3_ENDPOINT);
}
