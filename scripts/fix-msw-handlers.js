/**
 * Enhanced script to fix MSW v1 handlers to MSW v2 format
 * This script handles more complex patterns and edge cases
 */
const fs = require('fs');
...
// Function to update handlers with more comprehensive patterns
function updateHandlers(content) {
  // Replace standard GET handlers
  ...
  // Replace standard POST handlers
  content = content.replace(
  ...
  // Replace standard DELETE handlers
  content = content.replace(
  ...
  // Handle more complex multiline handlers with error responses
  content = content.replace(
    /rest\.(get|post|put|delete)\(['"]([^'"]+)['"]\s*,\s*\([^)]*\)\s*=>\s*{\s*return\s*res\(\s*ctx\.status\((\d+)\),\s*ctx\.json\(\s*{\s*success\s*:\s*false,\s*message\s*:\s*['"]([^'"]+)['"]\s*}\s*\)\s*\);\s*}\)/g,
    (match, method, url, status, message) => {
      return `http.${method}('${url}', () => {\n          return errorResponse('${message}', ${status});\n        })`;
    }
  );

  // Handle any remaining rest.* patterns with a simple replacement
  content = content.replace(
    /rest\.(get|post|put|delete)\(['"]([^'"]+)['"]/g,
    "http.$1('$2'"
  );
